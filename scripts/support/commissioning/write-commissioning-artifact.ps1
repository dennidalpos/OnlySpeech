...
$helpersPath = Join-Path $repoRoot "scripts\support\lib\plans.ps1"
. $helpersPath
...
$doctorResult = if ($SkipDoctor) {
  ...
  Invoke-CapturedPowerShellScript -ScriptPath (Join-Path $repoRoot "scripts\support\workspace\doctor.ps1")
}
