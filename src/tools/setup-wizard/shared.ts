...
export function getWizardConfigurationIssues(state: WizardState): WizardConfigurationIssue[] {
  ...
  const supportedWizardProviders = new Set<TranslationProvider>(["azure", "chatgpt"]);
  ...
  const commentCopyByLanguage: Readonly<Record<SetupWizardUiLanguage, Record<string, string>>> = {
    en: {
      ...
      providerCredentialScope:
        "# Fill only the credentials required by the selected translation provider before production use.",
      chatGptCredentials:
    ...
    it: {
      ...
      providerCredentialScope:
        "# Compila solo le credenziali richieste dal provider traduzione selezionato prima dell'uso in produzione.",
      chatGptCredentials:
    ...
    es: {
      ...
      providerCredentialScope:
        "# Completa solo las credenciales requeridas por el proveedor de traduccion seleccionado antes del uso en produccion.",
      chatGptCredentials:
    ...
    fr: {
      ...
      providerCredentialScope:
        "# Renseignez uniquement les identifiants requis par le fournisseur de traduction selectionne avant la production.",
      chatGptCredentials:
    ...
    de: {
      ...
      providerCredentialScope:
        "# Tragen Sie vor dem Produktionseinsatz nur die Zugangsdaten des ausgewaehlten Uebersetzungsanbieters ein.",
      chatGptCredentials:
    ...
    zh: {
      ...
      providerCredentialScope: "# 在生产使用前，只填写所选翻译服务商必需的凭据。",
